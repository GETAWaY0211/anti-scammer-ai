...
For Phase 5D-D, import `semantic-pattern-calibration-v1.json` and select **Semantic Pattern Lookup V1** in its Execute Workflow node. The harness passes only a fresh `{ context }` object into that strict sub-workflow; calibration labels stay outside and are reattached by `case_id = context.request_id`. Set `CALIBRATION_CASE_IDS` to `['prize_fee_02']` for the first smoke test, then restore `[]` for all 52 cases. The batch-size-one loop avoids concurrent embedding calls, but a full run still causes 52 query-embedding calls, so run it intentionally rather than on a schedule. Export only the normalized final result; do not export HTTP node data or raw embeddings. Process the export with `scripts/run-semantic-calibration.js`. The generated report is experimental calibration evidence, not a production threshold.

## 12. Phase 5D-E Main integration

Re-import or update **Text Analysis Main V2**, then open **Execute Semantic Pattern Lookup V1** and select the imported standalone lookup workflow. The exported reference is intentionally blank because n8n workflow IDs are installation-specific. Confirm the Gemini HTTP Header Auth credential on **Semantic Pattern Lookup V1 / Generate Query Embedding** and the Postgres credential on **PostgreSQL Cosine Lookup**, then save/publish the semantic dependency before Main.

Main sends only canonical text context to semantic lookup. Image Base64 is removed after extraction and never crosses this boundary. Semantic failures are optional and analysis continues with `semantic_pattern_intelligence.available = false`; Entity Intelligence failures remain authoritative `503` errors. Similarity remains internal supporting evidence only: it is not scam probability, does not create indicators, and does not affect scoring or the public response.

For manual validation, run a scam paraphrase, the legitimate negation `ธนาคารไม่มีนโยบายขอ OTP กรุณาอย่าส่งรหัสให้ผู้อื่น`, an investment warning, a benign message, and an image request. Inspect **Validate Semantic Pattern Result** and **Attach Semantic Pattern Intelligence** inside restricted execution data, then confirm the public response contains no similarity, vector, pattern, model, or database fields. Temporarily break the semantic credential to confirm Main still completes; restore it immediately. Separately stopping Entity Intelligence must continue to produce `503 INTELLIGENCE_LOOKUP_UNAVAILABLE`.

Phase 5D-F adds no credential or workflow reference. After re-importing Main, inspect **Evaluate Semantic Corroboration** after **Attach Semantic Pattern Intelligence**. A bank/OTP scam with validated `OTP_REQUEST` should internally corroborate `BANK_OTP_IMPERSONATION`; the legitimate OTP warning and investment-risk warning should not corroborate their similar patterns. Confirm the score is unchanged for an identical indicator set and that neither `semantic_corroboration` nor similarity data appears in the webhook response.
...
1. Import `n8n/workflows/speech-to-text-provider-v1.json`.
...
The trusted model is `gemini-3.6-flash`, selected inside **Build Gemini Audio Request**. Clients cannot provide a model, provider, URL, path, transcript, or vector. The workflow uses inline audio because Phase 6A limits decoded audio to 5 MiB, below Gemini's documented inline request limit. See the official [Gemini generateContent audio documentation](https://ai.google.dev/gemini-api/docs/generate-content/audio) for the provider transport format.
